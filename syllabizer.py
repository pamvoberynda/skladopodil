...
def separate_syllables(word: str, separator="-"):
    ...
    for syllable in correlated_syllables:
        output += syllable
        if add_sep_idx < lenght-1 and output[-1] != " ":
            output+= separator
        add_sep_idx+=1
    return output


worde = "що ж ти фраєре здав назад, не по масті я тобі. ти дивись в мої очі. кидай полошитись по долі"
